...
Thin HTTP client for the OpenRouter API. Handles:
...
class GatewayClient:
    """
    HTTP client for OpenRouter completions API.

    Responsibilities:
    - Send completion requests to OpenRouter
    ...
    def __init__(
        ...
        self.api_key = api_key or os.environ.get("NINE_ROUTER_API_KEY", "") or os.environ.get("OPENROUTER_API_KEY", "")
        if not self.api_key:
            raise GatewayConfigError(
                "LLM API key is required. "
                "Set NINE_ROUTER_API_KEY (or OPENROUTER_API_KEY) in .env"
            )
    ...
    def _execute_request(
        ...
        # OpenRouter/Anthropic cache stable system prompts at 50% discount
