...
def _call_domain_llm(
    ...
    Shared by all non-code nodes (research, product, hardware, telemetry).
    Uses 9Router to select appropriate model for the task type.

    Args:
        prompt: Full prompt with context + task.
        task_type: For 9Router routing (planning, documentation, coding, etc.)
        routing: Routing dict from state (may have pre-selected model).
        router: NineRouter (injected or loaded from config).
        gateway: GatewayClient (injected or created).
    ...
    try:
        ...
        log.error("domain_llm.failed", task_type=task_type, error=str(e))
        return None
