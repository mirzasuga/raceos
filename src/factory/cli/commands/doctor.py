...
def run_doctor(console: Console) -> None:
    ...
    checks = [
        ...
        ("API key set", _check_env("OPENROUTER_API_KEY")),
...
def _check_env(var: str) -> bool:
    import os
    # Check primary (9Router) or fallback (OpenRouter)
    val = os.environ.get("NINE_ROUTER_API_KEY", "") or os.environ.get("OPENROUTER_API_KEY", "")
    return len(val) > 5
