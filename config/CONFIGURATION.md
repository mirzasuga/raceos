...
| Key | Value | Why |
|---|---|---|
| `name` | `openrouter` | Gateway that provides access to all models via single API. |
| `baseUrl` | `https://openrouter.ai/api/v1` | OpenAI-compatible endpoint. |
| `apiKeyEnv` | `OPENROUTER_API_KEY` | Environment variable name (never hardcode keys). |
| `headers.X-Title` | `RaceOS Factory` | Identifies our app in OpenRouter dashboard. |
| `headers.HTTP-Referer` | `https://github.com/raceos` | Required by OpenRouter for attribution. |
