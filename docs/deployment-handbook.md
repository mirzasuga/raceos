...
|---|---|---|---|
| `NINE_ROUTER_API_KEY` | GitHub Secrets (env: `integration-test`) | E2E tests + scheduled integration | Yes |
...
|---|---|---|
| NINE_ROUTER_API_KEY | Every 90 days | Regenerate at 9router.ai, update GitHub |
...
"integration-test" environment:
  ...
  Secrets: NINE_ROUTER_API_KEY (budget-limited key, $1/day max)
```
...
Out of scope:
- Vulnerabilities in LLM providers (9Router, Anthropic, OpenAI)
...
```bash
cp .env.example .env
# Edit .env: add NINE_ROUTER_API_KEY (get from https://9router.ai/keys)
```
...
| API key leaked | Revoke at 9router.ai → regenerate → update .env |
...
| 9Router | LLM API (integration tests) | $1/day budget | For CI only |
