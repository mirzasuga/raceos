...
Project Brain (.raceos/)     [READ-ONLY knowledge]
   ...
   OpenCode (executor)       [Edit code via MCP tools]
       ↓
   9Router (gateway)      [HTTP proxy to LLM providers]
       ↓
...
LangGraph.invoke(initial_state)
  ...
  │     9Router.POST → DeepSeek classifies → {type:debugging, complexity:medium}
  ...
  │       └─► OpenCode internally calls 9Router → Claude
  ...
  │     9Router.POST → Claude reviews diff
...
│  │                  │                                              │
│  │  LLM Calls:     │                                              │
│  │    (internally)  → 9Router → 9Router-selected model         │
│  └────────┬─────────┘                                              │
│           │ stdout: JSON result                                    │
│           ▼                                                        │
│  ┌──────────────────┐                                              │
...
│           ▼                                                        │
│  ┌──────────────────┐                                              │
│  │ 9Router       │  Dumb HTTP pipe:                             │
│  │ (gateway)        │    POST https://9router.ai/api/v1/chat/  │
│  │                  │    Auth: Bearer $NINE_ROUTER_API_KEY          │
...
```
Human    CLI     Bridge   LangGraph  9Router  Brain   OpenSpec  Memory  OpenCode  9Router  LLM
  │       │        │         │         │       │        │        │        │          │        │
  │─cmd──►│        │         │         │       │        │        │        │          │        │
  │       │─req───►│         │         │       │        │        │        │          │        │
  │       │        │─invoke─►│         │       │        │        │        │          │        │
...
```
Engineer    9Router    9Router    LLM      Reviewer    9Router(retry)  9Router    LLM(better)
  │           │           │          │          │             │              │             │
  │─execute──►│           │          │          │             │              │             │
  │           │─Claude───►│─────────►│          │             │              │             │
  │◄──────────│◄──────────│◄─────────│          │             │              │             │
...
| 9Router | `gateway/client.py` + `retry.py` | HTTP + retry + fallback |
