...
|---|---|---|---|
| **OpenRouter** | https://openrouter.ai | Paling banyak model, mudah setup | Semua pengguna |
...
Kunjungi: https://openrouter.ai/keys
...
Config sudah default ke OpenRouter, tidak perlu ubah `router.yaml`.
...
```bash
OPENROUTER_API_KEY=sk-9r-xxxxxxxxxxxxxx
```
...
gateway:
  ...
  api_key_env: "OPENROUTER_API_KEY"         # tetap pakai env var yang sama
  headers:
...
```bash
# .env
OPENROUTER_API_KEY=sk-ant-xxxxxxxxxxxxxx
```
...
```
Baru mulai?                    → OpenRouter (paling mudah, semua model)
...
|---|---|
| OpenRouter | ~0% (pass-through pricing) |
...
**Ya** (jika pakai OpenRouter atau 9Router). Satu API key memberi akses ke Claude, GPT, Gemini, DeepSeek sekaligus.
...
|---|---|---|
| `OPENROUTER_API_KEY is required` | Key belum di-set | Tambahkan ke `.env` |
