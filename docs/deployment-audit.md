...
| 13 | `OPENROUTER_API_KEY` (CI) | Integration tests | ❌ Not configured in GitHub | HIGH |
...
| 25 | OpenRouter (sole LLM gateway) | Factory can't make LLM calls | Direct provider fallback (future) | MEDIUM |
...
| 29 | OpenRouter | Low | Gateway client is thin HTTP — swap to direct APIs | LOW |
...
| 41 | OpenRouter deprecates model IDs | Routing config breaks silently | MEDIUM |
...
| 25 | OpenRouter SPOF | Document direct-API fallback in config | 30 min |
...
Hour 2:
  ...
  □ Create budget-limited OpenRouter key for CI ($1/day)
  □ Add OPENROUTER_API_KEY to integration-test environment
